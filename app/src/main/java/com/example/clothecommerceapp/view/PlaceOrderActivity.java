...
public class PlaceOrderActivity extends AppCompatActivity implements PaymentResultListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        shippingDetailBack = findViewById(R.id.shippingDetailBack);
        ...
        String sAmount="100"; //just for test mode  or use cartpricetotal here in real scenario

        //convert and round off
        //for decimal value you round it off
        amount=Math.round(Float.parseFloat(sAmount)*100);

        confirmOrderButton();

        shippingDetailsBackImage();
    }

    private void shippingDetailsBackImage(){
        shippingDetailBack.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent=new Intent(PlaceOrderActivity.this, MyCartBagActivity.class);
    ...
    private void check() {
        ...
        //check if all the info is correct on edittext
    ...
    //add the key here everytime to see payment gateway because we are doing this in test mode
    private void paymentFunction(){

        Checkout checkout= new Checkout();

        //set key id
        checkout.setKeyID(""); //add the key here everytime to see payment gateway because we are doing this in test mode
    ...
    private void confirmOrderFunction() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                ...
                if(task.isSuccessful()) {
                    FirebaseDatabase.getInstance().getReference().child("Cart List")
                            ...
                            .removeValue()  //when you make order the cart product will be remove
